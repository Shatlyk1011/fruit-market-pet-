  ...
  <div class="w-1/2 relative" v-for="comment in formatDate" :key="comment">
    ...
    <div class="flex flex-col gap-1 items-start absolute bottom-0 right-6">
      <div class="relative">
        ...
        <div class="text-[9px] font-sans2 text-slate-200 font-light absolute top-[55%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-500 pointer-events-none select-none">{{comment.likes}}</div>
      </div>
      <div @click="handleDelete(comment.commentId)" class="text-[10px] text-red-400 hover:text-red-500 transition-all border ring-b-transparent hover:border-b-red-500 cursor-pointer active:scale-95 active:translate-y-0.5 select-none">удалить</div>
    </div>
  </div>
</template>
